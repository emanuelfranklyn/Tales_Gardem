...
module.exports = {
    Help: "Adiciona uma musica à lista de reprodução",
    ...
    Main: async (msg, args) => {
        ...
        let SearchResulted = await ytsr(args);
        Promise.resolve(SearchResulted).then(searchResults => {
            Promise.resolve(Waiting).then(function(value) {
                value.delete()
            }).catch(() => {
                console.log("Erro: Não foi possivel deletar: tocar.js: value.delete()")
            })
            ...
            if (size == 0) {
                ...
                for (var i=0; i < size; i++) {
                    //if (i !== 2) {
                        if (searchResults.items[i].type === "video" && searchResults.items[i].live === false) {
