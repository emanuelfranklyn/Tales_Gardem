...
module.exports = {
    ...
    Main: async (Client, msg, args) => {
        ...
        var CreationMessage;
        try {
			if (msg.channel.permissionsFor(Client.user).has("MANAGE_MESSAGES") == false) {
				const embed = new MessageEmbed()
                .setTitle('Sem permissão')
                .setColor(0xFF9999)
                .setThumbnail("https://cdn4.iconfinder.com/data/icons/user-management-soft/512/no_permission-512.png")
                .setDescription("Não tenho a permissão de gerenciar mensagens necessaria para executar este comando!"); 
                    ...
                    msg.channel.send(embed).then(async (message) => {
                        ...
                        const test = async (msg2) => {
                            ...
                            if (AddField == true) {
                                ...
                                if (msg2.toString().indexOf('#') == -1 || msg2.toString().length >= 8) {
                                    msg2.channel.send("Por favor insira somente um codigo Hex valido").then(msg154 => {
                                        FinalizationFormData.push(msg154)
                                        FinalizationFormData.push(msg2)
                                    })
                                } else {
                                ...
                                if (_G.ChannelToSend.permissionsFor(msg2.member).has("SEND_MESSAGES") && _G.ChannelToSend.permissionsFor(Client.user).has("SEND_MESSAGES")) {
                                    _G.FFENDER()
                                } else if (_G.ChannelToSend.permissionsFor(msg2.member).has("SEND_MESSAGES") == false && _G.ChannelToSend.permissionsFor(Client.user).has("SEND_MESSAGES")) {
                                ...
                                } else if (_G.ChannelToSend.permissionsFor(msg2.member).has("SEND_MESSAGES") && _G.ChannelToSend.permissionsFor(Client.user).has("SEND_MESSAGES") == false) {
                                    let RRRRRRORRE = msg.channel.send("Não tenho permissão para enviar mensagens nesse canal, insira outro na qual eu tenha permissão para falar!")
                                    FinalizationFormData.push(RRRRRRORRE)
                                }
                            } else {
                                ...
                                collector2.on('collect', async (reaction, reactionCollector) => {
                                    if (reaction.emoji.name == "🛑") {
                                        ...
                                        _G.FFCOLOR = () => {
                                            Visualization(true)
                                            FColor = true
                                            msg.channel.send("`Por favor insira o codigo HEX da cor da sua mensagem incorporada! Ex: #FFFFFF - Branco` - `Clique no 🔲 abaixo para deixar a cor aleatoria`").then(async (data) => {
                                                data.react("🔲")
                                                FinalizationFormData.push(data)
                                        ...
                                        _G.FFAUTHORN = () => { //CONTINUAR DAQUI!!!!!!!!!!!!!!!!!!!!!!!!
                                            ...
                                            //ask the author name
                                            if (msg.member.hasPermission('ADMINISTRATOR') == false) {
                                                TFAName = msg.author.username
                                                TFAImage = msg.author.avatarURL()
                                                TFAUrl = ""
                                                HaveAuthor = true
                                                _G.FFIMAGE() 
                                            } else {
