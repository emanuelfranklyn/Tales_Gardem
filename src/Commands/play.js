    ...
    Main: async (Client, msg, args) => {
        if ((args[1]!== undefined && typeof(args) !== typeof(""))|| typeof(args) !== typeof("")) {
            ...
            return msg.channel.send(embed).then((data)=> {
                ...
                const collector = data.createReactionCollector(filter, { time: 60000 });
                ...
                if (Data.queue && Data.queue[0]) {
                    ...
                    await sp.export('tocando agora: '+Data.queue[0].title, null, 1, path.resolve(CachePath,guild,"latestSpeak.ogg"), async (err) => {
                        ...
                        dispatcher1.on("finish", async () => {
                            ...
                            const dispatcher = await connection.play(await ytdl(Data.queue[0].url, {highWaterMark: 1<<25}), { type: 'opus'})
