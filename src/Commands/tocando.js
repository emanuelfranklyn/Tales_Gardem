...
module.exports = {
    //Base help
    Help: "Mostra o tempo de reprodução da música atual.",
    Usage: "tocando.",
    Topic: "media",
    ...
    Main: async (Client, msg, args) => {
        ...
        var status = "carregando";
            ...
            const verifyed = await cvs.loadImage("./images/verifiedicon.png");
            ctx.save();
            ...
            do {
            ctx.font = `${fontSize -= 9}px pirates of cydonia`;
            } while (ctx.measureText(Datanewer.queue[0].title).width > canvas.width / (1.45));
            ...
            if (Datanewer.queue[0].verified === true) {
                ctx.drawImage(verifyed, 270+ctx.measureText("Views: "+Datanewer.queue[0].author).width+10, canvas.height / 4.1, 20, 20 );
            }
            ...
            do {
                ctx.font = `${fontSize3 -= 3}px pirates of cydonia`;
            } while (ctx.measureText(shortdescp1).width > canvas.width / 1.05);
